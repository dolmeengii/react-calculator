...
import { runDebounceTest } from '@lib/debounceTest';
...
export default function DebouncePanel() {
  ...
  const handleRun = async () => {
    setLoading(true);
    const result = await runDebounceTest(20);
    setRows(result);
    setLoading(false);
  };
  ...
  return (
    <div className="mt-8 border rounded-lg p-4 max-w-xl mx-auto">
      <div className="flex items-center justify-between mb-4 gap-2">
        <h2 className="font-semibold text-lg">Debounce 성능 측정</h2>
        <button
          className="bg-blue-600 text-white px-3 py-1 rounded text-sm"
          onClick={handleRun}
          disabled={loading}
        >
          {loading ? '측정 중...' : 'Debounce 측정 실행'}
        </button>
      </div>
