...
import { runTransitionTest } from '@lib/transitionTest';
...
export default function TransitionPanel() {
  const [rows, setRows] = useState<TransitionResult[] | null>(null);
  const [loading, setLoading] = useState(false);

  const handleRun = async () => {
    setLoading(true);
    const result = await runTransitionTest(20);
    setRows(result);
    setLoading(false);
  };
  ...
  return (
    <div className="mt-8 border rounded-lg p-4 max-w-xl mx-auto">
      <div className="flex items-center justify-between mb-4 gap-2">
        <h2 className="font-semibold text-lg">Transition 성능 측정</h2>
        <button
          className="bg-purple-600 text-white px-3 py-1 rounded"
          onClick={handleRun}
          disabled={loading}
        >
          {loading ? '측정 중...' : 'Transition 측정 실행'}
        </button>
      </div>
